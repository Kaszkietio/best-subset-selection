...
def run_experiment(experiment: dict):
    ...
    path_dir = os.path.join(RESULTS_PATH, f"datasets//{experiment['data'].__name__}")
    os.makedirs(path_dir, exist_ok=True)

    pd.DataFrame(X).to_csv(os.path.join(path_dir, f"X_snr={data_params['snr']}.csv"), index=False)
    pd.DataFrame(y).to_csv(os.path.join(path_dir, f"y_snr={data_params['snr']}.csv"), index=False)
    pd.DataFrame(beta_true).to_csv(os.path.join(path_dir, f"beta_snr={data_params['snr']}.csv"), index=False)
...
def main():
    ...
    path_dir = os.path.join(RESULTS_PATH, "model_comparison")
    os.makedirs(path_dir, exist_ok=True)

    for snr, dataset in product(snr_list, data_config.keys()):
        config["data_params"] = data_config[dataset]
        config["data_params"]["snr"] = snr

        config["data"] = data_functions[dataset]
        k = k_features[dataset]
        config["mio_params"]["k"] = k
        config["dfo_params"]["k"] = k
        df_1 = run_experiment(config)
        df = pd.concat([df, df_1], axis=0, ignore_index=True)

    path = os.path.join(path_dir, "results.csv")
