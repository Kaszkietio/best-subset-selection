...
def main():
    ...
    for n_features, k in product(n_features_list, k0):
        config["mio_params"]["k"] = k
        config["dfo_params"]["k"] = k
        config["data_params"]["n_features"] = n_features
        df_1 = run_experiment(config)
        df = pd.concat([df, df_1], axis=0, ignore_index=True)

    path = os.path.join(path_dir, "results_different_p.csv")
